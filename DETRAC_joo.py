# 1. xml에서 필요한 정보 추출후, 한줄씩 txt파일에 쓰기, 시퀸스 단위로 작성(파일1개 = 1 시퀸스 라벨)
import os
line_num = 0 # 줄 세기
...
for target in raw_list:
    ...
    seperate_destination = os.path.join((os.path.join(out_dir, name)), 'gt' + '\\') # 시퀸스별로 저장시 경로 설정

    if not(os.path.isdir(seperate_destination)): # 시퀸스별 저장 경로 생성
        os.makedirs(seperate_destination)

    wrt = open(seperate_destination + 'gt' + '.txt', 'w') # 파일 쓰기
    ...
    while line:
        ...
        attribute_target = line.find('vehicle_type') # attribute 감지
        frame_target = line.find('num') # 프레임 위치 감지
        ex_target = line.find('box') # 좌표 감지
        target_ID = line.find('target id') # 트레킹 ID 감지
        ...
        if stop_igr_target > 0: # ignore 종료가 나올때 까지 작업 중지
        ...
        elif frame_target > 0 and igr == 0: # 현재 프레임 검색
            ...
            frame_num = (line[nu_st + 1:nu_ed])
            #frame_num = (line[nu_st + 1:nu_ed]).zfill(5)

        elif target_ID > 0 and igr == 0: # 현재 타겟 ID 검색

            finder()
            id_st = brace_list[0]
            id_ed = brace_list[1]

            id_num = str(line[id_st + 1:id_ed])

        elif ex_target > 0 and igr == 0: # bounding box 좌표 특정
        ...
        elif attribute_target > 0 and igr == 0: # class 검색
            ...
            attribute = line[att_st + 1:att_ed]
            obj_num += 1 # 디버깅용

            if attribute == 'others': # 실제 class
                others_num += 1
                appear_others = 1
                add_att_loca = 'others' # others를 요걸로 바꾸기

            elif attribute == 'car': # 실제 class
                num_car += 1
                appear_car = 1
                add_att_loca = 'car' # car를 요걸로 바꾸기

            elif attribute == 'van': # 실제 class
                others_num += 1
                appear_others = 1
                add_att_loca = 'others' # van를 요걸로 바꾸기

            elif attribute == 'bus': # 실제 class
                others_num += 1
                appear_others = 1
                add_att_loca = 'others' # bus를 요걸로 바꾸기

            #final = frame_num + ',' + id_num + ',' + add_att_loca + ',' + box + '1,0,0\n' # 클래스가 문자로 입력 + 1,0,0 // tracking용
            #final = frame_num + ',' + '-1' + ',' + box + '1,-1,-1,-1\n' # 클래스 무관 tracking용
            final = frame_num + ',' + id_num + ',' + box + '1,1,1\n' # GT
